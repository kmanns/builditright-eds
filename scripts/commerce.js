...
/**
 * Legacy BuildRight prototype paths mapped to ACCS storefront routes.
 * This enables phased migration from static `/pages/*.html` URLs.
 */
export const LEGACY_BUILDRIGHT_ROUTE_ALIASES = {
  '/login.html': CUSTOMER_LOGIN_PATH,
  '/signup.html': '/customer/create-account',
  '/account.html': CUSTOMER_ACCOUNT_PATH,
  '/order-history.html': CUSTOMER_ORDERS_PATH,
  '/cart.html': '/cart',
  '/catalog.html': '/products',
  '/product-detail.html': '/products/default',
  '/build-configurator.html': '/customer/requisition-lists',
  '/bom-review.html': `${CUSTOMER_PATH}/negotiable-quote`,
  '/project-builder.html': `${CUSTOMER_PATH}/requisition-list-view`,
  '/builder.html': `${CUSTOMER_PATH}/requisition-list-view`,
  '/dashboard.html': CUSTOMER_ACCOUNT_PATH,
  '/dashboard-templates.html': CUSTOMER_ACCOUNT_PATH,
  '/project-selector.html': CUSTOMER_ACCOUNT_PATH,
  '/pages/login.html': CUSTOMER_LOGIN_PATH,
  '/pages/signup.html': '/customer/create-account',
  '/pages/account.html': CUSTOMER_ACCOUNT_PATH,
  '/pages/order-history.html': CUSTOMER_ORDERS_PATH,
  '/pages/cart.html': '/cart',
  '/pages/catalog.html': '/products',
  '/pages/product-detail.html': '/products/default',
  '/pages/build-configurator.html': '/customer/requisition-lists',
  '/pages/bom-review.html': `${CUSTOMER_PATH}/negotiable-quote`,
  '/pages/project-builder.html': `${CUSTOMER_PATH}/requisition-list-view`,
  '/pages/builder.html': `${CUSTOMER_PATH}/requisition-list-view`,
  '/pages/dashboard.html': CUSTOMER_ACCOUNT_PATH,
  '/pages/dashboard-templates.html': CUSTOMER_ACCOUNT_PATH,
  '/pages/project-selector.html': CUSTOMER_ACCOUNT_PATH,
};

/**
 * Resolves legacy BuildRight prototype routes to ACCS storefront routes.
 * @param {string} path
 * @returns {string}
 */
export function normalizeLegacyBuildRightPath(path) {
  if (!path) return path;
  const normalized = path.endsWith('/') && path.length > 1 ? path.slice(0, -1) : path;
  return LEGACY_BUILDRIGHT_ROUTE_ALIASES[normalized] || path;
}
...
export function detectPageType() {
  if (document.body.querySelector('main .product-details')) {
    return 'Product';
  } if (document.body.querySelector('main .product-detail')) {
    return 'Product';
  } if (document.body.querySelector('main .product-list-page')) {
    return 'Category';
  } if (document.body.querySelector('main .product-grid')) {
    return 'Category';
  } if (document.body.querySelector('main .commerce-cart')) {
    return 'Cart';
  } if (document.body.querySelector('main .mini-cart')) {
    return 'Cart';
...
export function decorateLinks(main) {
  ...
  main.querySelectorAll('a').forEach((a) => {
    ...
    try {
      ...
      // Resolve legacy static prototype pages to canonical ACCS routes.
      const legacyResolvedPathname = normalizeLegacyBuildRightPath(pathname);
      if (legacyResolvedPathname !== pathname) {
        a.href = new URL(`${origin}${legacyResolvedPathname}${search}${hash}`);
        return;
      }
...
export function rootLink(link) {
  const normalizedLink = normalizeLegacyBuildRightPath(link);
  const root = getRootPath().replace(/\/$/, '');

  // If the link is already localized, do nothing
  if (normalizedLink.startsWith(root)) return normalizedLink;
  return `${root}${normalizedLink}`;
}
